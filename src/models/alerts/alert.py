...
class Alert(object):
    ...
    def __repr__(self):
        return "<Alert for {}: item {} with price limit {}".format(self.user_email, self.item.name, self.price_limit)

    def send(self):
        # print("Alert (to {}): the price of item {} (with limit {}) has changed to {}".format(self.user_email, self.item.name, self.price_limit, self.item.price))
    ...
    def find_needing_update(cls, minutes_since_update=AlertConstants.ALERT_TIMEOUT):
        ...
        # return all elems that haven't been checked for 10 minutes (ALERT_TIMEOUT)
        # ie: last_updated_limit >= self.last_checked
